...
|`Builder::build_phi(ty, name)` + `phi.add_incoming(&[...])`|`b.view(b.build_int_phi::<W, _>(name)?)` + `phi.add_incoming(value, block)?`|the builder returns a storable `PhiInstId<W>`; `b.view(..)` mints the typed handle. The phi starts with an empty incoming list; `add_incoming` checks each value against the phi's static `W` and returns the handle for chaining — mirrors `PHINode::addIncoming` for the loop-edge flow. `build_int_phi_dyn(ty, name)?`, `build_fp_phi::<K, _>(name)?`, and `build_pointer_phi(name)?` cover the other kinds. `phi.remove_incoming(&m, index)?` mirrors `PHINode::removeIncomingValue`|
