...
fn target_directives_round_trip_through_asm_writer() {
    let printed = Module::with_new("target_directives_round_trip", |m| {
        parse_into(
...
fn source_filename_round_trips_through_asm_writer() {
    let printed = Module::with_new("source_file", |m| {
...
fn top_level_comdat_round_trips() {
    let printed = Module::with_new("comdat_module", |m| {
...
fn global_externally_initialized_round_trips() {
    let printed = Module::with_new("global_externally_initialized", |m| {
...
fn global_linkage_round_trips() {
    let printed = Module::with_new("global_linkage", |m| {
...
fn global_visibility_round_trips() {
    let printed = Module::with_new("global_visibility", |m| {
...
fn global_dll_storage_round_trips() {
    let printed = Module::with_new("global_dll_storage", |m| {
...
fn global_tls_mode_round_trips() {
    let printed = Module::with_new("global_tls", |m| {
...
fn global_unnamed_addr_round_trips() {
    let printed = Module::with_new("global_unnamed_addr", |m| {
...
fn global_addrspace_round_trips() {
    let printed = Module::with_new("global_addrspace", |m| {
...
fn global_trailing_attributes_round_trip() {
    let printed = Module::with_new("global_trailing_attrs", |m| {
        parse_into(
...
fn module_asm_directives_accumulate() {
    let asm = Module::with_new("module_asm", |m| {
        parse_into(
...
fn named_struct_forward_reference_resolves() {
    Module::with_new("recursive_named", |m| {
...
fn array_and_vector_types_parse() {
    Module::with_new("aggregate_types", |m| {
...
fn scalable_vector_type_parses() {
    Module::with_new("scalable_vec", |m| {
...
fn variadic_declaration_round_trips() {
    let printed = Module::with_new("variadic_decl", |m| {
...
fn numbered_global_records_in_slot_mapping() {
    Module::with_new("numbered_globals", |m| {
...
fn void_in_value_position_is_rejected() {
    let err = Module::with_new("reject_void", |m| {
...
fn unknown_top_level_entity_is_typed_error() {
    let err = Module::with_new("unknown_top_level", |m| {
