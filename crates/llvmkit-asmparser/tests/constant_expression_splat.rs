...
/// Inserting at a lane other than 0 makes every shuffled lane `undef`, and the
/// whole vector folds to `undef`.
///
/// **This case is a regression test for a folder bug it found.**
/// `vector_splat_constant` built `min_len` elements with no scalability guard,
/// so this folded to `<vscale x 4 x ptr> <ptr undef, ptr undef, ptr undef, ptr
/// undef>` — an element list for a vector whose lane count is a *minimum*, not
/// a count. LLVM has no such constant and would reject the printed IR; llvmkit
/// was also quietly asserting `vscale == 1`. The scalable arm now answers with
/// the whole-vector spelling.
#[test]
fn a_scalable_splat_of_undef_stays_a_whole_vector_undef() {
    let module = parse(
        "@x = global i32 7\n\
    ...
    let printed = format!("{module}");
    assert!(
        printed.contains("@g = global <vscale x 4 x ptr> undef"),
        "a scalable vector has no element list to write:\n{printed}"
    );
    assert!(
        !printed.contains("ptr undef, ptr undef"),
        "the folder must not materialise a scalable vector:\n{printed}"
    );
}
