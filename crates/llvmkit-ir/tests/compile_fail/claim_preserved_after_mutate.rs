...
//! `cx.mutate()` consumes the context, the all-preserved `unchanged()` report is
...
//! the token/function/results into the rung's mutator; `FnCx::unchanged` also
//! takes `self` by value. So calling `cx.unchanged()` after `cx.mutate()` is a
...
impl<'ctx, B: ModuleBrand + 'ctx> FunctionPass<'ctx, B> for ClaimPreservedAfterMutate {
    ...
    fn run(&mut self, cx: FnCx<'_, '_, 'ctx, B, PatchBody, ()>) -> IrResult<FnReport> {
        ...
        // all-preserved `unchanged()` on the moved `cx` is unspellable: a mutating
