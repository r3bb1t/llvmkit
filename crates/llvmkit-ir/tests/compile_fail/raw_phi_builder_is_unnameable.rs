...
//! guarantee: `build_int_phi` is unnameable from outside the crate.
...
fn main() {
    Module::with_new("c", |m| {
        ...
        // `build_int_phi` is `pub(crate)` since slice 7 -- not nameable here.
        // The public path is `b.append_block_with_params(f, &[i32_ty], "bb")`.
        let _phi = b.build_int_phi::<i32, _>("p");
    });
}
