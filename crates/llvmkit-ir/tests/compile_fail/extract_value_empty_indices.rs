//! Compile-fail lock for `extract_value(agg, [], "x")` (Doctrine D3).
//! `ExtractValueInst::init` (`lib/IR/Instructions.cpp`) asserts a non-empty
//! index list at runtime; the typed `extract_value<V, const N: usize,
...
//! `extract_value_dyn` (which keeps the runtime check; see
