...
//!
//! A sealed block view is read-only rediscovery. It must not start a
//! lifecycle-producing `BlockCursor`.

use llvmkit_ir::{IRBuilder, Linkage, Module, iter::BlockCursor};

fn main() {
    Module::with_new("sealed-cursor", |m| {
        ...
        let (sealed, _ret) = b.build_ret_void();

        let _cursor = BlockCursor::at_start(sealed);
    });
}
