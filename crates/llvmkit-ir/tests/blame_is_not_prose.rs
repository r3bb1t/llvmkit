...
//! `Check` literals), raw-string literals, and the parser crate's messages.

use std::path::{Path, PathBuf};

use llvmkit_ir::{Blame, BrandError, IrError, TypeKindLabel};

const ERROR_RS: &str = include_str!("../src/error.rs");

/// Prose that spells a blame class, which `IrError::blame()` states as a value.
const BLAME_PHRASES: &[&str] = &[
    "internal invariant",
    "internal error",
    "should never",
    "never happen",
...
fn both_blame_answers_are_reachable() {
    ...
    assert_eq!(
        IrError::UnknownMetadataSlot { index: 0, len: 0 }.blame(),
        Blame::LlvmkitInvariant,
        "a slot past the arena under a matching module tag is llvmkit's own \
         bookkeeping failing: no caller can mint such an id"
    );
    assert!(
        blame_of_every_variant_is_stated(),
        "see the exhaustive match in error.rs"
    );
}

/// The exhaustive match is the guarantee; this names it so the assertion above
/// reads honestly rather than implying a runtime sweep.
fn blame_of_every_variant_is_stated() -> bool {
    true
}
...
fn blame_is_a_copy_value() {
    ...
    requires_copy::<Blame>();
    let _ = TypeKindLabel::Integer; // the import above is the vocabulary this sits in
}

fn spells_blame(literal: &str) -> bool {
    BLAME_PHRASES.iter().any(|phrase| literal.contains(phrase))
}
