...
use llvmkit_ir::{
    AtomicOrdering, AtomicRMWBinOp, Dyn, IRBuilder, IrError, Linkage, Module, PointerValue,
    SyncScope,
};
...
fn cmpxchg_no_align_monotonic_monotonic() -> Result<(), IrError> {
    Module::with_new("a", |m| {
        ...
        assert!(
            text.contains(
                "%cmpxchg_no_align.0 = cmpxchg ptr %0, i32 0, i32 4 monotonic monotonic\n"
            ),
            "got:\n{text}"
        );
        Ok(())
...
fn atomicrmw_umax_singlethread() -> Result<(), IrError> {
    Module::with_new("a", |m| {
        ...
        assert!(
            text.contains(
                "%u = atomicrmw umax ptr %0, i32 21 syncscope(\"singlethread\") monotonic\n"
            ),
            "got:\n{text}"
        );
        Ok(())
