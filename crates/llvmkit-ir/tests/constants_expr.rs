...
use llvmkit_ir::{
    ApInt, ConstantExprFlags, ConstantExprInRange, ConstantExprOpcode, Dyn, GepNoWrapFlags,
    IrBuilder, IrError, Linkage, Module, ModuleBrand, OverflowingConstantExprFlags, module_new,
};
...
fn constant_expr_bitcast_round_trips() -> Result<(), IrError> {
    let m = module_new!("constexpr_bitcast")?;
...
fn constant_expr_ptrtoaddr_round_trips() -> Result<(), IrError> {
    let m = module_new!("constexpr_ptrtoaddr")?;
...
fn token_none_round_trips() -> Result<(), IrError> {
    let m = module_new!("token_none")?;
    let none = m.token_none();
    let text = format!("{}", none.as_erased());
...
fn invalid_bitcast_constant_expr_is_rejected() -> Result<(), IrError> {
    let m = module_new!("constexpr_invalid_bitcast")?;
    let i32_ty = m.i32_type();
    let zero = i32_ty.const_int(0i32);
    let g = m.add_global("g", zero)?;
    let ptr = m.view(g).as_global_constant_ptr().as_erased();
...
fn invalid_shufflevector_constant_expr_non_i32_mask_is_rejected() -> Result<(), IrError> {
    ...
    let lhs = vec_i32_ty.const_vector::<llvmkit_ir::ConstantIntValue<'_, i32, _>, _>([one, two])?;
    let rhs =
        vec_i32_ty.const_vector::<llvmkit_ir::ConstantIntValue<'_, i32, _>, _>([three, four])?;
    let zero64 = i64_ty.const_zero();
    let one64 = i64_ty.const_int(1i64);
    let mask =
        vec_i64_ty.const_vector::<llvmkit_ir::ConstantIntValue<'_, i64, _>, _>([zero64, one64])?;

    let err = m
        .constant_expr(
            vec_i32_ty.as_type(),
            ConstantExprOpcode::ShuffleVector,
            [lhs.as_erased(), rhs.as_erased(), mask.as_erased()],
            [],
            [],
            ConstantExprFlags::none(),
        )
...
fn shufflevector_constant_expr_uses_mask_operand_when_folding() -> Result<(), IrError> {
    ...
    let rhs = src_ty.const_vector::<llvmkit_ir::ConstantIntValue<'_, i32, _>, _>([three, four])?;
    let zero = i32_ty.const_zero();
...
fn shufflevector_constant_expr_poison_and_scalable_undef_masks_fold() -> Result<(), IrError> {
    ...
    let rhs = fixed_ty.const_vector::<llvmkit_ir::ConstantIntValue<'_, i32, _>, _>([one, two])?;
    let fixed_mask = fixed_ty.as_type().poison().as_constant();
    let folded = m.constant_expr(
        fixed_ty.as_type(),
        ConstantExprOpcode::ShuffleVector,
        [lhs.as_erased(), rhs.as_erased(), fixed_mask.as_erased()],
        [],
        [],
        ConstantExprFlags::none(),
    )?;
    ...
    let scalable_mask = scalable_ty.as_type().undef().as_constant();
    let folded = m.constant_expr(
        scalable_ty.as_type(),
        ConstantExprOpcode::ShuffleVector,
        [lhs.as_erased(), rhs.as_erased(), scalable_mask.as_erased()],
        [],
        [],
        ConstantExprFlags::none(),
    )?;
    assert_eq!(folded, scalable_ty.as_type().poison().as_constant());
    Ok(())
}
...
fn scalable_shufflevector_zero_mask_is_accepted() -> Result<(), IrError> {
    ...
    let text = format!("{}", expr.as_erased());
...
fn empty_constant_expr_flags_are_canonicalized_before_interning() -> Result<(), IrError> {
    let m = module_new!("constexpr_empty_flags")?;
    let i32_ty = m.i32_type();
...
fn vector_gep_scalar_sequential_indices_are_splatted_before_interning() -> Result<(), IrError> {
    let m = module_new!("constexpr_vector_gep_splat")?;
    let i8_ty = m.i8_type();
    let i64_ty = m.i64_type();
    ...
    let gep = m.constant_expr_with_options(
        result_ty.as_type(),
        ConstantExprOpcode::GetElementPtr,
        [ptr.as_erased(), vector_index.as_erased(), zero.as_erased()],
        [],
        [],
        llvmkit_ir::ConstantExprOptions::new().source_ty(source_ty.as_type()),
    )?;
...
fn vector_gep_struct_index_width_mismatch_is_rejected() -> Result<(), IrError> {
    let m = module_new!("constexpr_vector_gep_struct_index_width")?;
    let i8_ty = m.i8_type();
    let i32_ty = m.i32_type();
    let i64_ty = m.i64_type();
...
///
/// Since `ConstantExprInRange` began holding two `ApInt`s rather than raw
/// words, this is `ApInt`'s own canonicalization doing the work — a 64-bit
/// `ApInt::from_words` keeps one word and drops the rest — but the property the
/// interner depends on is the same and is still worth pinning here: two ranges
/// spelled differently for the same value must intern to one constant.
#[test]
fn constant_expr_gep_inrange_words_are_truncated_before_interning() -> Result<(), IrError> {
    let m = module_new!("constexpr_inrange_canonical_words")?;
    let i8_ty = m.i8_type();
    ...
    let high_word_range = ConstantExprInRange::new(
        ApInt::from_words(64, &[0, u64::MAX]),
        ApInt::from_words(64, &[1, u64::MAX]),
    );

    let canonical = m.constant_expr_with_options(
        m.ptr_type(0).as_type(),
        ConstantExprOpcode::GetElementPtr,
        [ptr.as_erased(), offset.as_erased()],
        [],
        [],
...
fn constant_expr_gep_inrange_width_must_match_base_index_width() -> Result<(), IrError> {
    let m = module_new!("constexpr_inrange_width")?;
    let i8_ty = m.i8_type();
    let g = m.add_global("g", i8_ty.const_zero())?;
    let offset = m.i64_type().const_int(1i64);
    let wrong_width_range =
        ConstantExprInRange::new(ApInt::from_words(32, &[0]), ApInt::from_words(32, &[1]));
...
fn invalid_gep_constant_expr_address_space_mismatch_is_rejected() -> Result<(), IrError> {
    let m = module_new!("constexpr_invalid_gep_addrspace")?;
    let i8_ty = m.i8_type();
    let target = m
    ...
    let err = m
        .constant_expr_with_options(
            ...
            ConstantExprOpcode::GetElementPtr,
            [
                m.view(target).as_global_constant_ptr().as_erased(),
                vector_index.as_erased(),
            ],
            [],
            [],
...
fn ptrauth_constructor_requires_five_operand_shape() -> Result<(), IrError> {
    let m = module_new!("ptrauth_constructor")?;
    let i8_ty = m.i8_type();
    let g = m.add_global("g", i8_ty.const_zero())?;
    let ptr = m.view(g).as_global_constant_ptr();
    ...
    let constant_expr_pointer = m.constant_expr_with_options(
        m.ptr_type(0).as_type(),
        ConstantExprOpcode::GetElementPtr,
        [addr_disc.as_erased(), disc.as_erased()],
        [],
        [],
        llvmkit_ir::ConstantExprOptions::new().source_ty(i8_ty.as_type()),
    )?;
