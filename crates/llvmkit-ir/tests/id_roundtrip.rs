...
fn ids_are_copy_and_send() {
    ...
    fn id_bounds<'ctx, B: ModuleBrand + 'ctx>(_m: &'ctx Module<B, Unverified>) {
    ...
    let m = module_new!("id-copy-send").expect("fresh module");
    id_bounds(&m);
}
...
/// error, so it never reaches the runtime comparison. Two
...
fn every_id_is_an_erased_operand() {
    ...
    let m = module_new!("id-erased-operand").expect("fresh module");
    let i32_ty = m.i32_type();
    let f32_ty = m.f32_type();
    let ptr_ty = m.ptr_type(0);
    let fn_ty = m.fn_type(
