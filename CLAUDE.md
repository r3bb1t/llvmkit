...
`handle.id()` mints the public tagged id; `m.view(id)` / `m.try_view(id)` goes the other way. No slot leaves the crate. Inside it, a handle's or id's slot comes out only through two crate-private doors: `slot_in(owner)`, which compares modules and refuses another module's with its `Foreign*` error (an id's checked route is `ViewIn::resolve_in` / `into_stored`, which its `slot_in` is built on), and `slot_trusting_same_module()`, for reads that provably stay in one module (on `ValueId` and the metadata ids it exists only for the crate-private `StoredBrand`). `BlockId` has no unchecked door: the crate reads its own CFG's block slots through slot-level queries (`InstructionView::parent_slot`, `FunctionCfg::predecessor_slots`), and its one exception is the named door `slot_unchecked_at_marked_boundary()`. The infallible entries and read-only analyses that cannot refuse yet read a caller's slot unchecked under a counted `// boundary (F1)` / `// boundary (F2)` marker — a `BlockId` through that named door, which `tests/boundary_door_drift.rs` fails on when a call sits under no marker. Declarations and value-producing builders return **ids**. Block appenders and terminator builders are the deliberate exceptions — they return linear `!Copy` handles, because their job is to be consumed exactly once.

A fourth shape sits beside the three: a **witness**, a value whose existence is the proof of a precondition. `PlacedInstruction<'ctx, B>` is the one that ships — an `InstructionView` paired with the block it was in — and the five entries that insert or move relative to an instruction take it, so "this instruction was never in a block" is a compile error rather than a run-time refusal (`InstructionView::parent` is `Option`, porting the null `Instruction::getParent`). A witness has no public constructor; it is minted checked (`InstructionView::placed`), totally (`Instruction<Attached>::placed`), or checklessly by the walk that already knows the block (`BasicBlock::placed_instructions` / `BasicBlockView::placed_instructions`). Know its ceiling before reaching for the pattern again: llvmkit mutates through `&Module` with interior mutability, so a witness proves *was placed*, never *is placed* — the entries re-read the current block and still refuse a stale witness. Only exclusive access freezes mutable state, which is why the stronger form is a layout session and is recorded, unbuilt, in `docs/future-work.md`.
...
- **Public enums are exhaustive. No `#[non_exhaustive]`, anywhere.** It buys non-breaking additions by handing every downstream `match` a `_ =>` arm, and on an error type that arm is where a caller is forced to describe an outcome it has no vocabulary for — a 56-variant return once obliged the parser's brand mapper to write an arm for 54 unreachable variants, and it filled that arm by stuffing a stringified `IrError` into an I/O error with `ErrorKind::Other`. Adding a variant is a breaking change, flagged inline in `CHANGELOG.md` under the pre-1.0 policy; that is the intended signal, not a cost to engineer around. Model a genuinely unbounded namespace with an open **variant** (`MetadataAttachmentKind::Custom`), never an open enum.
...
- **A premise in a brief is a check that will not happen.** Where code lives, where a value came from, what a routine is reached from, how many edits a fix needs, what an existing entry covers — in a plan, task spec, handoff or review finding these are claims someone else will act on without re-deriving them. Verify and paste the evidence beside the claim, or convert the premise into the executor's first step. **Do not hedge it instead** — a hedge moves your uncertainty to someone with less context. Every observed failure came from an agent who had *already read* the refuting code; three shipped in one session, two contradicted by a block already on screen. Run every command a brief contains before the brief ships: that is how a `sed` anchor off by one line, and a type claimed in the wrong crate, were both caught. Procedure: the `briefs-and-premises` skill.
...
- about to write a plan, task spec, handoff or review finding another agent will execute — anything stating where code lives, what a value came from, what a routine is reached from, or what an existing entry covers → `briefs-and-premises`
