...
```bash
cargo +1.96.0 test --workspace --all-targets --all-features    # full suite (2543 tests, 214 binaries)
...
- `UPSTREAM.md` — per-test provenance registry. Coverage is **not** total and the header says so: 2543 tests, 2110 rows, 320 tests with no row, all inherited from the type-safety and pass-API programs. A missing row means missing *provenance*, never "no upstream counterpart".
