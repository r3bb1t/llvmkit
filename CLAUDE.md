...
- **No silent erasure.** A typed handle or id never widens to an erased one implicitly. Erasure is spelled `as_dyn()` or a `_dyn` method. `_dyn` means exactly one thing — the erased half of a typed/erased pair.
- **Full words, no abbreviations**: `instruction` not `inst`, `predecessor` not `pred`. Internal indices are `*Slot`; public tagged ids are `*Id`. Do not blur them.
