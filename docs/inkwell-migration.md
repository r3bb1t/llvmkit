...
|`CallSiteValue::get_operand_bundles()`|`m.view(call).operand_bundles()` / `.operand_bundle(&tag)?`|on `CallInst`, `InvokeInst` and `CallBrInst`; each yields `OperandBundleUse` views whose `inputs()` are `Value`s. `operand_bundle` refuses a tag the call carries twice (`InvalidOperation`) where upstream's `getOperandBundle` asserts|
