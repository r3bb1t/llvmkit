...
|"the instruction I am positioning before is in a block"|no check at all — `Builder::position_before(&self, instruction: &InstructionValue)` takes any instruction and returns `()`, and LLVM dereferences the null `getParent()` of one that was built without an insert position or removed from its block|`IrBuilder::position_before` takes a `PlacedInstruction<'ctx, B>`, which has no public constructor, so an instruction that was never in a block is `E0308` (locked by `tests/compile_fail/position_before_needs_a_placed_instruction.rs`). Mint one with `InstructionView::placed` (checked), `Instruction<Attached>::placed` (total), or `BasicBlock::placed_instructions` (checkless). The residue is staleness — a detach between the mint and the call is refused with `IrError::InstructionHasNoParent`|
...
|`OperandBundle::create(tag, &[values])`|`OperandBundleDef::new(OperandBundleTag::…, [values])`|the tag is an enum (`OperandBundleTag::Deopt`, …, `Custom(String)` for the rest); the inputs stay value handles, each carrying its module|
|`Builder::build_direct_call_with_operand_bundles(fn, &[args], &[bundles], name)`|`b.call_builder(callee).arg(a).operand_bundles([bundle]).name(name).build()?`, or `CallSiteConfig::new(name).operand_bundles([bundle])` on `call_erased` and the `_with_config` invoke / callbr entries|bundles go beside the arguments, as `IRBuilderBase::CreateCall(…, Args, OpBundles, …)` takes them, never inside `CallAttributeData`; an input of another module is refused with `IrError::ForeignValueId` before the call is created|
|`CallSiteValue::get_operand_bundles()`|`m.view(call).operand_bundles()` / `.operand_bundle(&tag)?`|on `CallInst`, `InvokeInst` and `CallBrInst`; each yields `OperandBundleUse` views whose `inputs()` are `Value`s. `operand_bundle` refuses a tag the call carries twice (`DuplicateOperandBundle`) where upstream's `getOperandBundle` asserts|
...
|`Builder::position_before(&instruction)`|`IrBuilder::new_for::<R>(&m).position_before(witness)?`|takes a `PlacedInstruction` rather than an `&InstructionValue`, and returns `IrResult` — `ForeignValueId` for an anchor from another module, `InstructionHasNoParent` for one detached after the witness was minted. `Instruction::move_before` / `move_after` / `insert_before` / `insert_after` take the same witness|
|—|`block.placed_instructions()`|new — walks a block's instructions yielding witnesses directly, so the common `for` loop over a block needs no `Option` before positioning|
