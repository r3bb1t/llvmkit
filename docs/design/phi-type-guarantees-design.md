...
> **Shipped as.** `IRBuilder::append_block_with_params(function, param_types: &[Type], name) -> IrResult<BlockWithParams<'ctx, R, B>>`, where `BlockWithParams` is `(BasicBlock<'ctx, R, Unterminated, B>, Vec<Value<'ctx, B>>)` — the block comes back as the linear, non-`Copy` insertion handle that still owes a terminator, and `.id()` mints the storable `BlockId`. A naming twin, `append_block_with_named_params`, takes `(type, name)` pairs so block-argument authoring can reproduce named-phi output byte-for-byte. Only two of the three argument-carrying terminators landed: `build_br_with_args` and `build_cond_br_with_args`. There is **no** `build_switch_with_args` and no invoke variant. The arity and type guards live on the `*_with_args` builders themselves (`IrError::PhiArgArityMismatch`, `IrError::TypeMismatch`, both checked up front so a bad edge leaves no half-formed terminator); a *plain* `build_br` / `build_cond_br` aimed at a parameterised block is **not** rejected at the branch builder — it simply adds no incomings, and the resulting incomplete phi is caught by `Module::verify()` (`PhiEmptyInReachableBlock` and the shared `check_phi` count guard) or, for `.ll` input, at parse time. The typed layer that arrived later (`feature-20/typed-block-params`) covers the same two edges: `append_block_typed::<Params>` plus `BlockCall` consumed by `build_br_call` / `build_cond_br_call`, where a wrong-arity or wrong-typed block argument is a compile error.
- ~~Plain `build_br`/`build_cond_br` to a param-block → immediate arity error.~~ **Designed, not shipped** — see the "Shipped as" note above: `build_br` resolves its target and records the edge with no param-count check at all (`ir_builder.rs::build_br`), so this degraded from a build-time error to a `verify()` / parse-time one. `indirectbr` to a param-block → error (documented restriction; classic phi-with-indirectbr still parses from `.ll`).
