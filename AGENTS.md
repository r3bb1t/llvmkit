...
This file is the **exhaustive reference**: API laws, translation idioms,
per-workstream history, and the porting anchor tables for each LLVM subsystem.
`CLAUDE.md` is the *operating manual* — what to run, how the pieces fit, and
which rules fail CI or a review — and is deliberately a condensation of this
file. When the two disagree, check the tree and fix whichever is stale.
...
2. **Provide an `IRBuilder` analog** (llvmkit's `IrBuilder`) for programmatic IR construction.
...
### The handle model — read this before writing new API

The per-workstream summaries further down are a *historical* record, and parts
of them predate the 0.0.4 reshape. Where the two disagree, this subsection
wins. (`CLAUDE.md` carries the same section under the same name, condensed.)
...
- Declarations and **value-producing** builders return ids —
  ...
  `int_add -> IrResult<IntValueId<W, B>>`,
  `call -> IrResult<TypedCallInstId<Ret, B>>`. **Not every builder does:**
...
- Lookups are bare nouns (C-GETTER — no `get_` prefix), and each returns the
  ...
  `function_dyn -> Option<FunctionId<Dyn, B>>`, and
  `function::<R> -> IrResult<Option<FunctionId<R, B>>>` (a signature
  mismatch stays `IrError::ReturnTypeMismatch`, never a widened id). All are
  state-generic and take `&self`. `comdat -> Option<ComdatRef<'ctx, B>>` is
...
- Internal arena indices are named `*Slot` (`ValueSlot`, `TypeSlot`,
  ...
  `BlockId` has no unchecked door; a caller's block id read at a counted
  `// boundary (F1)` / `// boundary (F2)` site goes through the named
  exception `slot_unchecked_at_marked_boundary()`, pinned to those markers
  by `tests/boundary_door_drift.rs`.
- **Phis are not authored by hand.** `IrBuilder::int_phi` /
  ...
  `br_with_args` / `cond_br_with_args` / `switch_with_args` /
  `switch_dyn_with_args` / `invoke_with_args` /
  `invoke_dyn_with_args`, or the compile-checked `br_call` /
  `cond_br_call` — which is the one to reach for. A plain terminator
  ...
  (`phi_dyn`, `int_phi_dyn`, `fp_phi_dyn`,
  `pointer_phi_in_addrspace`) are public builder methods.
...
- The full medium IrBuilder: every integer binop (`add`/`sub`/`mul`/`udiv`/`sdiv`/`urem`/`srem`/`shl`/`lshr`/`ashr`/`and`/`or`/`xor`) plus per-opcode flag types (`AddFlags`/`UdivFlags`/...), every float binop (`fadd`/`fsub`/`fmul`/`fdiv`/`frem`), every cast (`trunc`/`zext`/`sext`/`bitcast`/`ptrtoint`/`inttoptr`/`fptrunc`/`fpext`/`fptosi`/`fptoui`/`sitofp`/`uitofp`/`addrspacecast`), `icmp`/`fcmp`, control flow (`br`/`cond_br`/`unreachable`), phi (**not** hand-authored: the typed constructors and `add_incoming` are `pub(crate)`; the public routes are block parameters, `SsaBuilder`, `FnReshape::insert_phi`, and the `_dyn` escape hatches — see **The handle model**), memory (`alloca`/`load`/`store`: the flat forms plus the `alloca_builder` / `load_from` / `store_to` chains), `getelementptr` (`gep`/`inbounds_gep`/`struct_gep`), `call` (flat + chainable `CallBuilder`), and `select` (sealed `SelectArm` for int/float/pointer arms).
...
- **Mutation API (T1)**: full instruction-lifecycle typestate. `Instruction<'ctx, S, B>` is parameterised by `S: state::InstructionState` (sealed; variants `Attached` / `Detached`) — there is no default for `S`. `Instruction` is intentionally **`!Copy` and `!Clone`** (Doctrine D2): the linear-typed handle prevents use-after-erase / double-erase at compile time. Copyable per-opcode handles (`AddInst`, `LoadInst`, ...) expose `as_view(&self) -> InstructionView` for read-only inspection; lifecycle mutation requires a builder-produced `Instruction<Attached>` or `iter::BlockCursor` rediscovery. Open variable-arity handles (`SwitchInst<Open>`, `IndirectBrInst<Open>`, `LandingPadInst<Open>`, `CatchSwitchInst<Open>`) are also linear and consumed by their mutators / `finish`, so retained open handles cannot mutate after finalisation. Every operand slot in `instr_types.rs` is wrapped in `core::cell::Cell<ValueSlot>` (and `Cell<Option<ValueSlot>>` for the optional `alloca` / `ret` operands) so RAUW can rewrite the operand wiring through `&self`. The shipped lifecycle methods are:
    ...
    - `Instruction<Attached>::move_before(self, &Module<B, Unverified>, PlacedInstruction) -> IrResult<()>` / `move_after` --- `Instruction::moveBefore` / `moveAfter`.
    - `Instruction<Detached>::insert_before(self, &Module<B, Unverified>, PlacedInstruction) -> IrResult<Instruction<Attached>>` / `insert_after` / `append_to(&Module<B, Unverified>, block)` --- `Instruction::insertBefore` / `insertAfter` / `insertInto`.
    - **The placement witness.** Those four and `IrBuilder::position_before` take `PlacedInstruction<'ctx, B>` --- an instruction paired with the block it was in --- so an instruction that was never in a block cannot reach them (D1). It has no public constructor; the mints are `InstructionView::placed` (checked, `Option`), `Instruction<Attached>::placed` (total --- that typestate is only minted in a block), and `BasicBlock::placed_instructions` / `BasicBlockView::placed_instructions`, which need no check because the block is the one being walked. A witness proves *was placed*, never *is placed*: llvmkit mutates through `&Module` with interior mutability, so the five entries re-read the current block and refuse a witness stale by a `detach_from_parent` with `IrError::InstructionHasNoParent`. Freezing that would take exclusive access to the function's layout, which is unbuilt (`docs/future-work.md`).
    ...
    - `iter::BlockCursor::at_start(block)` and `cursor.step() -> Option<(Instruction<Attached>, BlockCursor)>` --- the canonical advance-then-mutate iteration helper (Doctrine D9). Yields each instruction by value while consuming the cursor; the next call sees the precomputed snapshot, so erasing the yielded handle does not invalidate the iteration.
...
- **Numerics and analysis — the largest body of work after the IR model, and
  ...
  **The parity ledger is the authority, not this file.**
  ...
  every unconsulted arm named in `known_fp_class.rs`'s module header.
...
The const-generic `VectorType<E, Len<N>>` / `ArrayType<E, ArrLen<N>>` retrofit (T4 follow-up) **shipped** for fixed-length vectors and arrays (`feature-17`; element + length markers, `vector_int_*` / `vector_extract` / `vector_insert` / `vector_splat` / `array_extract` / `array_insert` making element/length mismatch a compile error). Its residual stays erased / `Dyn`: **scalable vectors** (always `Dyn`), **pointer-element vectors** (blocked on address-space markers), **composite-element arrays** (`[N x {..}]` / `[N x [..]]` / `[N x <..>]`), **float / div / rem vector binops and vector `icmp`/`fcmp`** (no erased `_dyn` lowering to reuse), and the length-relating ops (shufflevector output length, concat `N1+N2`, compile-time index-in-bounds, cross-`Len` widen/narrow) that are **blocked on `generic_const_exprs` (unstable)**. Still ahead beyond that: a broader optimization-transform library (`DcePass`, a fold-to-constant `InstSimplifyPass`, and `SimplifyDemandedBitsPass` ship today); a **runnable** textual pass pipeline — `pass_pipeline.rs` parses a pipeline string into typed data, but there is no NAME→constructor registry, so nothing can execute one, and that is the gating item for the transform work; bitcode; debug info; and fuller intrinsics coverage. See [`docs/future-work.md`](docs/future-work.md) for the full session-end roadmap (killer-feature designs, upstream coverage gaps, ergonomics backlog, and this session's own punted items).
...
- `crates/llvmkit-tablegen/` — the sixth workspace member: a port of LLVM's `lib/TableGen` front end plus the `utils/TableGen/Basic` intrinsic emitter, and the vendored `tablegen/llvm-22.1.4/` `.td` inputs it reads. `llvmkit-ir`'s `build.rs` calls it; it is a crate rather than an inlined build script so its own modules mirror the upstream files they port and its tests run under `cargo test`.

Feature flags: `llvmkit` and `llvmkit-ir` both declare `default = ["macros"]`
(the umbrella's `macros` forwards to `llvmkit-ir/macros`, which gates the
user-facing macro re-exports only — the proc-macro crate compiles either way).
`default-members = ["llvmkit"]`.
...
├── LICENSE
├── AGENTS.md                        # this file — the exhaustive reference
├── CLAUDE.md                        # the operating manual: what to run, which rules gate CI
├── docs/                            # user guides + design records; see docs/README.md
│   ├── inkwell-migration.md         # per-API delta against inkwell
│   ├── type-safety-vs-llvm.md       # the main technical reference
│   ├── ir-struct-derive.md          # #[derive(IrStruct)] user guide
│   ├── future-work.md               # the live backlog, with the reason per deferral
│   └── design/                      # dated records — history, not current API docs
...
└── crates/
    ...
    │   ├── src/
    │   │   ├── lib.rs               # generate() / vendored_llvm_root()
    │   │   ├── main.rs              # table_gen_main() — the standalone CLI driver
    │   │   ├── tg_lexer.rs          # lib/TableGen lexer
    │   │   ├── tg_parser.rs         # lib/TableGen parser
    │   │   ├── record.rs            # record values + the def/class evaluation engine
    │   │   └── basic/               # utils/TableGen/Basic intrinsic emitter
    │   └── tablegen/llvm-22.1.4/    # vendored .td inputs (Intrinsics*, ValueTypes, SDNodeProperties)
    ├── llvmkit-ir/                  # IR data model
    │   ├── Cargo.toml
    │   ├── build.rs                 # calls llvmkit_tablegen::generate()
    │   ├── src/
    │   │   ├── lib.rs
    ...
    │   │   │
    │   │   │                        # --- numerics and analysis (see the ledger below) ---
    │   │   ├── ap_int.rs            # APInt.h/.cpp
    │   │   ├── ap_float.rs          # APFloat.h/.cpp — seven modeled semantics
    │   │   ├── data_layout.rs       # DataLayout.h/.cpp
    │   │   ├── constant_range.rs    # ConstantRange.h/.cpp
    │   │   ├── constant_folding.rs  # Analysis/ConstantFolding.cpp
    │   │   ├── intrinsics.rs        # intrinsic ids/signatures (generated from tablegen/)
    │   │   ├── target_library_info.rs # TargetLibraryInfo.h
    │   │   ├── known_bits.rs        # KnownBits.h/.cpp — complete, ledger asserts no gaps
    │   │   ├── demanded_bits.rs     # DemandedBits.h/.cpp
    │   │   ├── value_tracking.rs    # ValueTracking.cpp — computeKnownBits and the value predicates
    │   │   ├── select_pattern.rs    # the SelectPatternResult vocabulary + matching
    │   │   ├── pointer_analysis.rs  # getUnderlyingObjects, string info, object size
    │   │   ├── speculation.rs       # isSafeToSpeculativelyExecute, poison/UB reachability
    │   │   ├── assumptions.rs       # @llvm.assume + AssumptionCache/DomConditionCache
    │   │   ├── implied_conditions.rs # isImpliedCondition and its dominating-branch form
    │   │   ├── fp_class.rs          # FPClassTest + KnownFPClass lattice
    │   │   ├── known_fp_class.rs    # computeKnownFPClass + its predicates
    │   │   └── fp_predicate.rs      # fcmpImpliesClass / fcmpToClassTest
    ...
    │   └── tests/                   # 135 integration files; a few anchors:
    │       ├── asm_writer_basic.rs
    │       ├── vertical_slice.rs
...
4. **D4.** Result types reflect operand types (`int_add::<i32, _, _, _>` yields an `IntValueId<i32, B>`).
...
- the static-marker form (`int_load::<W>(ptr, name) -> IrResult<IntValueId<W, B>>`);
- the erased fallback (`int_load_dyn(ty, ptr, name)` takes an explicit `IntType<'ctx, IntDyn, B>` and returns `IrResult<IntValueId<IntDyn, B>>`).

Same posture for trunc / zext / sext / fp loads. (Phi is the exception: the pair exists, but only the `_dyn` half is public — see the phi bullet under **The handle model**.) The dyn form keeps the runtime check; the typed form pins the invariant at compile time. `_dyn` marks the `Dyn`-marker member of such a pair and nothing else; where a third, fully-erased shape exists (`Value` operands plus a runtime opcode — the vector-capable forms), it is spelled `_erased` (`int_binop_erased`, `fp_cmp_erased`). Never abbreviate an unrelated name to either suffix, and never suffix the typed form — the full three-tier vocabulary is the naming law under **Code Conventions**.
...
- `b.call(typed_callee, (a, b), "r")?` for schema-typed construction — the argument tuple is compile-checked position-by-position against the callee's parameter schema.
...
- **Naming law: full words, no abbreviations.** Spell it out — `instruction`, not `inst`; `predecessor`, not `pred`; `metadata`, not `md` — in every new public and internal name. Upstream's abbreviated spellings are not a licence to copy them. The sanctioned suffix vocabulary is **three-tier**: the typed form carries **no suffix** (`int_add`, `int_load::<W>`); **`_dyn`** marks the `Dyn`-marker member of a typed/erased pair — the form whose result or operand marker is `IntDyn` / `FloatDyn` / `LenDyn` / `Dyn` (`int_load_dyn`, `call_dyn`); **`_erased`** marks the fully-erased third tier — `Value` operands, plus a runtime opcode or predicate where the family has one; these are the vector-capable forms (`int_binop_erased`, `fp_cmp_erased`, `select_erased`). Never abbreviate an unrelated name to `_dyn` or `_erased`, and never suffix the typed form. Internal arena indices are `*Slot` (`ValueSlot`, `TypeSlot`, `MetadataSlot`); the public tagged ids are `*Id`. Do not blur the two.
- **A public enum variant that shares a name with a public type un-trims that type's diagnostics.** rustc prints a type's *short* name in an error only while that name resolves to exactly one item in the crate; add a variant with the same spelling and every diagnostic mentioning the type — in this crate and in every downstream one — switches to the fully-qualified path. This is not cosmetic in a repo with blessed `.stderr` fixtures: naming a `VerifierSubject` variant `Module` moved 12 spellings of `Module<B, S>` to `llvmkit_ir::Module<B, S>` across **8** `tests/compile_fail/*.stderr` files, none of them about verification, and the diff looks exactly like the "environmental trybuild drift" this file elsewhere says does not exist. It is not drift; it is the rename. The variant became `WholeModule` and all 87 fixtures went green with no re-blessing. Twenty such collisions already exist and are fine because the types were already un-trimmed when the fixtures were blessed — derive the list with the type-vs-variant scan rather than copying it (`^pub (enum|struct) ([A-Z]\w+)` over `crates/llvmkit-ir/src` intersected with the variant names inside each `pub enum`; 482 public types, 874 variant names, 20 collisions at `325dd26`). The rule is not "never collide": it is **check the intersection before choosing the name**, and if a fixture diff shows only a path lengthening, look for a new variant before touching a `.stderr`.
- **Lookup law (C-GETTER): lookups are bare nouns.** A by-name or by-key lookup is the noun itself — `global(name)`, `alias(name)`, `ifunc(name)`, `comdat(name)`, `function::<R>(name)`, `function_dyn(name)`, `named_struct(name)`, `named_metadata(name)` — never `get_<noun>`. The `get_` prefix survives in exactly one shape, the std-consistent get-or-insert entry points, mirroring `HashMap`-family precedent; there are seven, and the list is closed: `get_or_insert_comdat`, `get_or_insert_intrinsic_declaration`, `get_or_insert_intrinsic_declaration_by_id`, `get_or_insert_intrinsic_declaration_by_name`, `get_or_insert_named_metadata`, `get_or_insert_named_struct`, `get_or_insert_struct_of`. Accessors never take a `get_` prefix either (`field()`, not `get_field()`).
...
- **Compile-time invariants on cast widths** flow through sealed marker traits. `int_width::WiderThan<W>` is implemented for every `(Wider, Narrower)` pair of static markers, and the IR builder uses it as a bound on `trunc<Src: WiderThan<Dst>, Dst>` (and inversely on `zext` / `sext`). The `_dyn`-flavoured fallbacks keep the runtime check for the genuinely-erased path.
...
not optional hygiene, and it is not automatic: there is **no
`rust-toolchain.toml`** in the repo, so a bare `cargo` picks up whatever
default toolchain is installed. `rust-version = "1.96"` in
`[workspace.package]` is the MSRV declaration, not a pin. Spell the `+1.96.0`
every time. The trybuild `.stderr` fixtures under
...
The CI gate is a *subset* of that list, in this order
(`.github/workflows/ci.yml`): `fmt -- --check`; a per-package license check
(`cmp LICENSE <dir>/LICENSE` for all six members, so a new crate needs its own
copy of the root `LICENSE`); `check --workspace --examples`; `clippy`; `doc`
under `RUSTDOCFLAGS=-D warnings`; `test --workspace --all-targets
...
- Conventional Commits: `type(scope): summary` — `feat`, `fix`, `docs`, `test`, `refactor`, `chore`. Append `!` after the scope for a breaking change (`refactor(ir,asmparser)!: lookups are bare nouns`). The scope names the workstream or cycle slice.
...
- `.gitignore` — ignores `/target`, `/orig_cpp/`, `/build/`, plus the local tooling scratch dirs (`/.worktrees/`, `/.superpowers/`, `/docs/superpowers/`). `Cargo.lock` is **committed** (the workspace ships binaries / examples).
...
2. **Read the current signature, not a doc's memory of it.** The API froze at 0.0.4 after a broad reshape; any prose — including this file's historical workstream summaries — can lag the tree. `grep` the `pub fn` before you call it, and check **The handle model** above before choosing between an id, a view, and a borrowing handle.
