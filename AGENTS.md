...
- **Prefer imports over fully qualified paths.** Import types, traits, and helper functions at the top of the module (`use crate::metadata::MetadataKind;`) instead of spelling `crate::metadata::MetadataKind` inline. Fully qualified paths are reserved for one-off disambiguation, macro hygiene, or cases where an import would create a real name conflict.
