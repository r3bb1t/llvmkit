...
└── crates/
    ...
    │   └── tests/                   # 129 integration files; a few anchors:
    │       ├── asm_writer_basic.rs
    │       ├── vertical_slice.rs
...
- **Lookup law (C-GETTER): lookups are bare nouns.** A by-name or by-key lookup is the noun itself — `global(name)`, `alias(name)`, `ifunc(name)`, `comdat(name)`, `function::<R>(name)`, `named_struct(name)`, `named_metadata(name)` — never `get_<noun>`. The `get_` prefix survives in exactly one shape, the std-consistent get-or-insert entry points (`get_or_insert_named_struct`, `get_or_insert_struct_of`, `get_or_insert_comdat`), mirroring `HashMap`-family precedent. Accessors never take a `get_` prefix either (`field()`, not `get_field()`).
