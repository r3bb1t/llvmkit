...
- **Public API shape**: user-facing alternate operations are separate methods, not `Option<T>` inputs (`set_*`/`clear_*`, `*_within_none`, `*_to_caller`). Public config/result structs keep fields private and expose constructors plus C-GETTER-style accessors (`field()`, never `get_field()`). Public signatures use imported type names instead of `crate::...` / `super::...`.
